...
class StatData(object):
    ...
    def load_gude_data(self):
        ...
        for it in gd_list:
            ...
            if wid not in rs_map:
                rs_map[wid]={}
            date=datetime(2017,03,16)
            inner_key = '2017-03-16'
            if inner_key not in rs_map[wid]:
    ...
    def get_tr_rs(self):
        ...
        for k,v in rs.items():
            for kk,vv in v.items():
                ...
                #vv.update(gu_item)
                if k in rs_gude and kk in rs_gude[k]:
                    vv.update(rs_gude[k][kk])
                else:
                    vv.update(rs_gude_default)
